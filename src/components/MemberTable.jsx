    ...
    <div>
      ...
      <div className="bg-obsDark text-obsGray border border-obsRed p-4 rounded-lg">
        ...
        <div className="flex flex-wrap gap-2 my-2">
          {classList.map((cls) =>
            classCounts[cls] ? (
              <span key={cls} onClick={() => setFilterClass(cls)}
        ...
        <div className="flex gap-2 mb-2 text-xs">
          ...
          <button onClick={() => { setSortBy(null); setFilterClass(null); }}
            className="bg-gray-600 px-2 py-1 rounded text-white">🔁 Zurücksetzen</button>
        </div>
      ...
      <ul className="space-y-1 mt-4">
        {visibleMembers.map((member) => {
          ...
          return (
            <li key={member.id}
              className={`flex flex-col sm:flex-row justify-between gap-2 p-2 rounded text-white text-sm ${classColors[member.class] || "bg-gray-700"}`}>
              <div className="flex flex-col gap-1">
                <div className="font-semibold">{member.name}</div>
                <div className="flex flex-wrap gap-2 items-center text-xs">
                  Level:
                  <input type="number" value={member.level}
                    onChange={(e) => updateMember(member.id, "level", e.target.value)}
                    className="w-14 px-1 py-0.5 rounded text-black text-xs" />
                  Klasse:
                  <select value={member.class}
                ...
                <div className="flex flex-wrap gap-1 mt-1">
                  {weekKeys.slice(-3).map((week) => (
                    <span key={week} onClick={() => toggleWeek(member.id, week)}
                      className={`px-2 py-0.5 rounded cursor-pointer text-xs ${member.paidWeeks?.[week] ? "bg-green-600" : "bg-red-600"}`}>
                      {week}
                    </span>
                  ))}
                </div>
