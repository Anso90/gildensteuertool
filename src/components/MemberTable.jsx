...
export default function MemberTable({ members, setMembers, taxConfig }) {
  ...
  useEffect(() => {
    ...
    fetchInactive();
  }, [members]); // neu: updated wenn sich members ändern
  ...
  const updateMember = async (id, field, value) => {
    ...
    const { error } = await supabase.from("members").update({ level, class: klasse }).eq("id", id);
    if (error) console.error("❌ Fehler beim Speichern in Supabase:", error.message);
  };
  ...
  const toggleWeek = async (id, week) => {
    ...
    member.paidWeeks = { ...member.paidWeeks, [week]: !current };
    setMembers(updated);
    await setPaymentStatus(id, week, !current);
  };
  ...
  if (sortBy === "level") visibleMembers.sort((a, b) => b.level - a.level);
  else if (sortBy === "class") visibleMembers.sort((a, b) => a.class.localeCompare(b.class));
  ...
  return (
    <div>
      ...
      <div className="bg-obsDark text-obsGray border border-obsRed p-4 rounded-lg">
        <div>👥 <strong>Gesamtmitglieder:</strong>{" "}
          <span onClick={() => setFilterClass(null)} className="cursor-pointer text-white">{members.length}</span>
        </div>
        <div className="flex flex-wrap gap-2 my-2">
          {classList.map((cls) =>
            classCounts[cls] ? (
              <span key={cls} onClick={() => setFilterClass(cls)} className={`cursor-pointer px-2 py-1 rounded text-white text-xs ${classColors[cls]}`}>
                {cls}: {classCounts[cls]}
              </span>
            ) : null
          )}
            ...
            <li key={member.id} className={`flex flex-col sm:flex-row justify-between gap-2 p-2 rounded text-white text-sm ${classColors[member.class] || "bg-gray-700"}`}>
              <div className="flex flex-col gap-1">
                ...
                <div className="flex flex-wrap gap-2 items-center text-xs">
                  Level:
                  <input type="number" value={member.level} onChange={(e) => updateMember(member.id, "level", e.target.value)} className="w-14 px-1 py-0.5 rounded text-black text-xs" />
                  Klasse:
                  <select value={member.class} onChange={(e) => updateMember(member.id, "class", e.target.value)} className="px-1 py-0.5 rounded text-black text-xs">
                    {classList.map((cls) => <option key={cls} value={cls}>{cls}</option>)}
                  </select>
                  Steuer: <strong>{calculateTax(member.level)}</strong>
                </div>
                {taxText && <div className="text-yellow-200 text-xs italic">{taxText}</div>}
              </div>
            </li>
          );
        })}
